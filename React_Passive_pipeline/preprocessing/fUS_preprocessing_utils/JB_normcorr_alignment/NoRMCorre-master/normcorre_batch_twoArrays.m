function [M_final,shifts_g,template,options,col_shift] = normcorre_batch_twoArrays(Y_in,Y_out,options,template)
...
%{
%%%
    This is an alteration that I, Jeffrey Boucher, am making, beginning on 18/11/2021.
    The goal is to calculate the shifts with one matrix, and apply the shifts to another
    matrix. The purpose is to be able to filter my fUS data in one manner to calculate the
    shifts (focusing perhaps on tissue) and to apply the shifts to data filtered in another
    manner, for anlysis purposes (perhaps fUS data with the large blood vessels filtered out).
    
    Mainly this is going to mean a lot of renaming variables and a lot of doing things twice.
%%%
%}
%% first determine filetype
%%%
%{
  Mine will always be loaded into memory... So I am going to ignore a lot of "Y".
  I've commented out the if statements and replaced the values with "0" to always
  skip them but leave them there to see if I need them later.
%}
%%%
if 0%isa(Y,'char')
    [~,~,ext] = fileparts(Y);
...
%%%
%{
    As we are getting the template, we only need Y_in here. So little changes have been made.
%}
%%%
...
switch filetype
    ...
    case 'mat'
        if nd == 2; Y_temp = Y_in(:,:,interval); elseif nd == 3; Y_temp = Y_in(:,:,:,interval); end %%% Y_temp seems to only be used for the template, so it can be Y_in solely.
    case 'raw'
...
%{
    _s means start. _f means finish.
%}
...
%{
      Jeffrey here to say that the above is probably concerned very specifically with the
      upsampled. Furthermore, it deals with the overlap: that's what the _ov signifies.
%}
...
for i = 1:length(xx_us)
    for j = 1:length(yy_us)
        for k = 1:length(zz_us)
            ...
            %{
              Jeffrey here. Bs seems to apply "1" to everything within the actual frame and slope off
              for things more distant, which is likely an implementation of the fft interpolation,
              one way or another.
            %}
...
%{
      Jeffrey here. This is an interesting cell array. It stores the templates at each patch.
      One can clearly see the overlap here. I should return here with my own data.
%}
    ...
    for t = 1:bin_width:T
        switch filetype
            ...
            case 'mat'
                %%%
                %{
                    Need one Ytm each for Y_in and Y_out... The shifts will be applied to "Yt" which is gathered
                    within the Ytm pipeline. So until that happens I'll have two coppies of each Y matrix, for
                    Y_in and Y_out.
                %}
                %%%
                if nd == 2; Ytm_in = single(Y_in(:,:,t:min(t+bin_width-1,T))); end
                if nd == 3; Ytm_in = single(Y_in(:,:,:,t:min(t+bin_width-1,T))); end
                if nd == 2; Ytm_out = single(Y_out(:,:,t:min(t+bin_width-1,T))); end
                if nd == 3; Ytm_out = single(Y_out(:,:,:,t:min(t+bin_width-1,T))); end
            case 'raw'
                Ytm = single(read_raw_file(Y,t,min(t+bin_width-1,T)-t+1,FOV,bitsize));
        end
        %%%
        %{
            Currently, this creation of cell arrays makes no sense to me at all.
            There seems to be no advantage? I guess maybe it is useful because of
            potential differences in "nd", but I feel like that could be written around
            more easily...
        %}
        %%%
        if nd == 2; Ytc = mat2cell(Ytm_in,d1,d2,ones(1,size(Ytm_in,ndims(Ytm_in)))); end
        if nd == 3; Ytc = mat2cell(Ytm_in,d1,d2,d3,ones(1,size(Ytm_in,ndims(Ytm_in)))); end
        Mf = cell(size(Ytc)); %%% this is where the output goes I think. I can likely use either Y_in or Y_out to define its size.
        Mf_in = cell(size(Ytc));
        lY = length(Ytc);
        ...
        for ii = 1:lY %%%///%%% Once parfor
            %{
              %%%
                  So this will iterate through my frames and both calculate and apply shifts...
          
                  I want to hack it to apply the shifts to a different matrix. I suspect the easiest
                  way to do this is to load both arrays in at once and handle them together... Therefore
                  I will need to make lots of changes to this which means that I need to maybe make a new
                  program...
            ...
            %}
            Yt = Ytc{ii};
            Yt_out = Ytm_out(:,:,ii); %%% this should always be equivalent to the cell array... Maybe the cell array was for the parfor? Who knows.
            minY = min(Yt(:)); %%% WARNING this is apparently used to bound your data!!! So I need one of each for Yt and Yt_out
            maxY = max(Yt(:));
            minYt_out = min(Yt_out(:)); %%% WARNING this is apparently used to bound your data!!! So I need one of each for Yt and Yt_out
            maxYt_out = max(Yt_out(:));
            Yc = mat2cell_ov(Yt,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY);
            %%%
            %{
                Jeffrey here. Yc is pre-upsampling stuff. It also is specific to whichever
                frame you are working on. It's analogous to "template" and will be matched to
                it. All of this sounds obvious, but this is here for clarity.
            %}
            %%%
            if use_windowing
            ...
            if numel(M_fin) > 1
                %%%
                %{
                    My perception of this is that it is the registration for the entire image. The other ones are
                    registrations for the patches... Furthermore, I think there is good news here because it looks like
                    the outputs are just the shifts. Which means I bet we input them elsewhere.
                %}
                %%%
                if use_windowing
                    ...
                    %%%
                    %{
                        The output of this block is a bit hidden. You want the
                        variables "lb" and "ub" which almost certainly stand for
                        "lower bound" and "upper bound" They also appear to, by design,
                        contain exactly the same information. Not sure what is up with that.
                        Both are out_rig(3:4).
                    %}
                    %%%
                ...
                for j = 1:length(yy_s)
                    for k = 1:length(zz_s)
                        ...
                        if all([length(xx_s),length(yy_s),length(zz_s)] == 1) && strcmpi(options.shifts_method,'fft');
                            %%%
                            %{
                                This is it! The thing that was Yt, I changed into Yt_out.
                            
                                Actually it isn't it, because my shift method is 'linear' not 'fft'
                            %}
                            %%%
                            M_fin{i,j,k} = shift_reconstruct(Yt_out,shifts_temp(i,j,k,:),diff_temp(i,j,k),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},options.boundary,add_value);
                        end
                    end
                end
            end
            ...
            switch lower(options.shifts_method)
                ...
                otherwise
                    ...
                    if nd == 3
                        ...
                        %%%
                        %{
                            This is actually it, this time. Changed Yt to Yt_out.
                        %}
                        %%%
                        Mf{ii} = imwarp(Yt_out,-cat(3,shifts_up(:,:,2),shifts_up(:,:,1)),options.shifts_method,'FillValues',fill_value);
                        Mf_in{ii} = imwarp(Yt,-cat(3,shifts_up(:,:,2),shifts_up(:,:,1)),options.shifts_method,'FillValues',fill_value);
                    end
            end
            Mf{ii}(Mf{ii}<minYt_out)=minYt_out;
            Mf{ii}(Mf{ii}>maxYt_out)=maxYt_out;
            Mf_in{ii}(Mf_in{ii}<minY)=minY;
            Mf_in{ii}(Mf_in{ii}>maxY)=maxY;
        end
        
        shifts_g(t:min(t+bin_width-1,T)) = shifts;
        Mf = cell2mat(Mf);
        Mf_in = cell2mat(Mf_in);
        ...
        if upd_template
            %%%
            %{
              If I want to use update template, I must also do a template matching
              for Y_in...
            %}
            %%%
            cnt_buf = cnt_buf + 1;
            if nd == 2; buffer = mat2cell_ov(Mf_in,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY); end
            if nd == 3; buffer = mat2cell_ov(Mf_in,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY); end
