...
%% plot shifts        
  %{
    Jeffrey comments:
      ...
      So here's the way the shifts are organized. We have: patches and
      upsampled patches. We have corresponding shifts. Within shifts2,
      "shifts" refers to the former, "shifts_up" refers to the latter. 
      Each of these variables is 4d. The first 2 dimensions correspond to
      each patch size. The forth dimension is x movement (1) or y movement (2). 
      The third dimension is probably used for 3d data. 

      This understanding cannot account for overlapping. It also cannot account
      for the upsampled patches, as in this demo, shifts_up has the same dimensions as
      shifts. 

      "diff" within the shifts2 struct stores phase differences, which is to
      say "output(2)" of dftregistration_min_max_3d. 
  %}
