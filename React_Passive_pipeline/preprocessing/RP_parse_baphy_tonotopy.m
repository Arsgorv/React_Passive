function B = RP_parse_baphy_tonotopy(datapath)
% RP_parse_baphy_tonotopy
...
%   B.cat_id            : numeric cat id (e.g. 668, 650...)
%   B.frequence         : '200', '400' etc 
%   B.is_silence        : 0/1
...
if isempty(m_list)
    ...
    if isempty(m_list)
        error('RP_parse_baphy_tonotopy:NoMfile','No *.m Baphy file found in %s',baphy_dir);
    end
end
if numel(m_list) > 1
    warning('RP_parse_baphy_tonotopy:MultipleMfile','Multiple m-files, using first: %s', m_list(1).name);
end

m_file = fullfile(m_list(1).folder, m_list(1).name);
disp(['[RP_parse_baphy_tonotopy] Running m-file: ' m_file]);
...
if ~exist('exptevents','var')
    error('RP_parse_baphy_tonotopy:NoExptevents','exptevents not found after running %s',m_file);
end
...
for it = 1:n_trials
    ...
    % ---- NEW frequency-based classification ----
    nm = B.stim_name{it};
    
    B.cat_id(it)   = str2num(nm);     % numerical value (Hz)
    B.frequence{it} = nm;     % string label
